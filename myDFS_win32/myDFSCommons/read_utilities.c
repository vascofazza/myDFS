...
ssize_t writen_nb(SOCKET fd, const void *vptr, size_t n) {
    ...
    while (nleft > 0) {
        ...
        if (res < 0 && errno != EINTR) {
            ...
            if ((nwritten = send(fd, ptr, nleft, 0)) <= 0) { return (nwritten); }        /* error  <0*/
...
ssize_t my_read_nb(SOCKET fd, char *ptr, read_struct *r) {
    if (r->read_cnt <= 0) {
        int res = poll_read(fd);
        if (res < 0 && errno != EINTR) {
            ...
            // Socket selected for read
            if ((r->read_cnt = recv(fd, r->read_buf, MAXLINE, 0)) == SOCKET_ERROR) {
...
ssize_t writen(int fd, const void *vptr, size_t n) {
    ...
    nleft = n;
    while (nleft > 0) {
        if ((nwritten = write(fd, ptr, nleft)) <= 0) { return (nwritten); }        /* error  <0*/
        nleft -= nwritten;
        ptr += nwritten;
    }
    return (n);
...
ssize_t my_read_sock(SOCKET fd, char *ptr, read_struct *r) {
    if (r->read_cnt <= 0) {
        // Socket selected for read
        if ((r->read_cnt = recv(fd, r->read_buf, MAXLINE, 0)) < 0) {
            //if (errno == EINTR){ DOVREI GIA' GESTIRLO
            return (-1);
        } else if (r->read_cnt == 0) {
            return (0);
...
ssize_t my_read(int fd, char *ptr, read_struct *r) {
    if (r->read_cnt <= 0) {
        // Socket selected for read
...
ssize_t read_block(int fd, void *vptr, unsigned long block_no, size_t block_size) {
    ...
    if (lseek(fd, pos, SEEK_SET) < 0)
        return -1;
    return readn(fd, vptr, block_size);
}
...
ssize_t peek(SOCKET fd, void *ptr, size_t size, read_struct *r)
{
    ...
    ssize_t res = recv(fd, ptr, size, MSG_PEEK) + r->read_cnt;
    if (!SetNonBlockingSocket(fd, TRUE))
    {
        return -1;
    }

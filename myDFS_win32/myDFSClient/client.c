...
int nonblocking_connect(MyDFSId *id, SOCKET fd, int timeout);
                        ...
                        switch (current_token++) {
                            ...
                            case 2:
                            {
                                ...
                                HANDLE hMap = CreateFileMapping(INVALID_HANDLE_VALUE, //use paging file
                                                              ...
                                                              0, // hi-size
                                                                id->n_blocks == 0? id->config.block_size:id->n_blocks*id->config.block_size, //low-size
                                                              NULL
                                );
                                if (hMap == NULL)
                                {
                                    my_perror("creating file mapping");
                                    return -1;
                                }

                                void *map= MapViewOfFile(hMap, FILE_MAP_ALL_ACCESS, 0,0,0);
                                if(map == NULL)
                                {
                    ...
                    case DFS_SEND_CODE:
                    {
                        ...
                        switch (current_token++) {
                            case 0:
                            {
                                ...
                                SOCKET receive_sock = id->data_fd;
                                ...
                                //r MI SERVE PER IMPEDIRE CHE IL FILE VENGA AMPLIATO ED AVERE CASINI
                                ssize_t res = readn_nb(receive_sock, block_ptr, r);
                                if (res != r)
                                {
                                    if (res == -3)
                                        return -3; //timeout
//                                    my_perror("receive_block");
                                    return -1;
                                }
                                cache[block] = 1;
...
//    }
    ...
    if (!SetNonBlockingSocket(fd, TRUE))
    {
        my_perror("error setting non-blocking socket");
        return -1;
    }
...
//        return -1;
//    }
    if (!SetNonBlockingSocket(fd, FALSE))
    {
        my_perror("error setting non-blocking socket");
        return -1;
    }
...
ssize_t nonblocking_send_block(MyDFSId *id, long block_no)
{
    ...
    {
        my_perror("MyDFS - send_query");
        return -1;
    }
    if(nonblocking_receive_response(id) != DFS_OK_CODE)
    {
        my_perror("MyDFS - receive_response");
        return -1;
    }
...
MyDFSId *mydfs_open(const char *host, const char *nomefile, const int modo, int *err)
{
    ...
    {
        id->thread = CreateThread(NULL, 0, &(heart_beating_thread), id, 0, NULL);
        if (id->thread == NULL)
        {
            *err = errno;
            my_perror("MyDFS - thread init");
            return NULL;
        }
...
int mydfs_close(MyDFSId *id)
{
    ...
    free(id->cache);
    if (!UnmapViewOfFile(id->mapping))
    {
        my_perror("MyDFS - munmap");
        semsignal(sem);
        return -1;
    }
    close_socket(&id->ctrl_fd); //TODO PRIMA O DOPO?
    close_socket(&id->data_fd);
    //smappo
    //termino il thread
    if (id->thread != NULL)
    {
        HANDLE thread = id->thread;
        id->thread = NULL;
        //su linux se interrompeva semwait non veniva ucciso
//        if(pthread_cancel(id->thread) < 0)
        if (WaitForSingleObject(thread, INFINITE) == WAIT_FAILED)//pthread_join(thread, NULL) < 0)
        {
            semsignal(sem);
            return -1;
        }
...
{
    if (!(id->fmode & (MDFSO_RDONLY | MDFSO_RDWR | MDFSO_CREAT)))// && id->mode != 0)
        return -1;
...
{
    if (!(id->fmode & (MDFSO_WRONLY | MDFSO_RDWR)))
        return -1;
...
ssize_t file_resize(MyDFSId *id, ssize_t size)
{
    ...
    if(new_n_blocks > id->allocated_blocks)
    {
        ...
        HANDLE hMap = CreateFileMapping(INVALID_HANDLE_VALUE, //use paging file
                                        ...
                                        PAGE_READWRITE, //Access
                                        0, // hi-size
                                        doubled*id->config.block_size, //low-size
                                        NULL
        );
        if (hMap == NULL)
        {
        ...
        void *new_map= MapViewOfFile(hMap, FILE_MAP_ALL_ACCESS, 0,0,0);
        if(new_map == NULL)
        {
            my_perror("MapViewOfFile - remap");
            return -1;
        }
        close_handle(&hMap);
        int i = 0;

...
void TEST3()
{
    ...
    if (pthread_create(&thread1, NULL, &myfunc, NULL) != 0)
    {
        perror("thread init");
        return;
    }
    if (pthread_create(&thread2, NULL, &myfunc, NULL) != 0)
    {
        perror("thread init");
        return;
    }
    
    if (pthread_create(&thread3, NULL, &myfunc, NULL) != 0)
    {
        perror("thread init");
        return;
    }
    if(pthread_join(thread3, &returnvalue) < 0 || (intptr_t)returnvalue < 0)
    {
        perror("thread join");
        return;
    }
    if(pthread_join(thread1, &returnvalue) < 0 || (intptr_t)returnvalue < 0)
    {
        perror("thread join");
        return;
    }
    
    if(pthread_join(thread2, &returnvalue) < 0 || (intptr_t)returnvalue < 0)
    {
        perror("thread join");
        return;
    }
...
void beating()
{
    ...
    char path[100];
    unsigned seed = 0;
    sprintf(path, "Desktop/dfs_test/dfs_prova.mydfsWrite%d.pdf", rand_r(&seed));
    int err = 0;
    ...
    sprintf(path, "Desktop/dfs_test/prova.mydfsWrite%d.pdf", rand_r(&seed));
    int err1 = 0;
